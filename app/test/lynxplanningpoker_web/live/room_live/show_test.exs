defmodule LynxplanningpokerWeb.RoomLive.ShowTest do
  ...
  describe "end_planning / leave_room events" do
    ...
    test "presence-leave cleanup is deferred by the grace period", %{conn: conn} do
      # A short, non-zero grace so the deferral is observable; the suite
      # otherwise runs presence cleanup synchronously (grace 0).
      Application.put_env(:lynxplanningpoker, :leave_grace_ms, 300)
      on_exit(fn -> Application.put_env(:lynxplanningpoker, :leave_grace_ms, 0) end)
      ...
      # Right after the leave, nothing is cleaned up yet — it is deferred.
      assert Users.get_user!(bob.id)

      # Bob never returns, so once the grace window elapses he is removed.
      Process.sleep(450)
      _ = render(view)
      assert_raise Ecto.NoResultsError, fn -> Users.get_user!(bob.id) end
    end
