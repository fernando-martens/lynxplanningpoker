defmodule LynxplanningpokerWeb.RoomControllerTest do
  ...
  describe "GET /rooms/new" do
    ...
    test "is marked noindex so search engines skip the form page", %{conn: conn} do
      ...
      assert response =~ ~s(<meta name="robots" content="noindex)
      refute response =~ ~s(<link rel="canonical")
    end
    ...
    test "renders the Turnstile widget when enabled", %{conn: conn} do
      ...
      # The submit button starts disabled — it is enabled client-side only
      # once Turnstile passes, so an early click can't submit unverified.
  ...
  describe "GET /rooms/leave" do
    ...
    test "redirects to the localized home for a non-default-locale session",
         %{conn: conn} do
      {:ok, room} = Rooms.create_room(%{})
      {:ok, user} = Users.create_user(%{room_id: room.id, name: "Amélie"})

      # A player using the app in French leaves the room — they must land on
      # the French home, not the prefix-free (default-locale) one.
      conn =
        conn
        |> Plug.Test.init_test_session(%{current_user_id: user.id, locale: "fr"})
        |> get(~p"/rooms/leave")

      assert redirected_to(conn) == "/fr"
    end
  end

  describe "locale-prefixed room flow" do
    test "GET /fr/rooms/new renders the create-room form in French", %{conn: conn} do
    ...
    test "GET /fr/rooms/invite/:id renders the invite page in French", %{conn: conn} do
      {:ok, room} = Rooms.create_room(%{})
      response = conn |> get(~p"/fr/rooms/invite/#{room.id}") |> html_response(200)

      assert response =~ ~s(<html lang="fr">)
      assert response =~ ~s(action="/fr/rooms/invite/#{room.id}")
    end
