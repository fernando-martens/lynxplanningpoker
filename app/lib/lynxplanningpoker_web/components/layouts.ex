defmodule LynxplanningpokerWeb.Layouts do
  ...
  `page_path` is the current page's default-locale path (e.g. `/pricing`). When
  given, the language switcher links straight to the localized URL of the same
  page; when omitted (live rooms, which have no crawlable per-locale URL) it
  falls back to the session-based `/locale/:locale` switch.
  """
    ...
    <div class="relative" phx-click-away={JS.hide(to: "#settings-panel")}>
      ...
      >
        ...
        <div class="flex items-center justify-between gap-3">
          ...
          <.language_switcher page_path={@page_path} />
        </div>
      </div>
    </div>
    """
  ...
  On public pages (`page_path` given) each option links directly to the
  localized URL of the current page — distinct, crawlable URLs are what let
  search engines index every language. On live rooms (`page_path` omitted) it
  falls back to `/locale/:locale`, which stores the choice in the session and
  bounces back via the `Referer` header.
  """
  attr(:page_path, :string, default: nil)

  attr(:id, :string,
    default: "language-options",
    doc: "id of the options list — unique per instance"
  )
  ...
  def language_switcher(assigns) do
    ...
    ~H"""
    <div class="relative" phx-click-away={JS.hide(to: "##{@id}")}>
      <button
        type="button"
      ...
      >
        <li :for={other <- @others}>
          <.link
            href={language_href(other, @page_path)}
  ...
  # Both forms route through `/locale/:locale`, which persists the choice in
  # the session (so live rooms inherit it) before redirecting. On public pages
  # `return_to` carries the localized URL of the current page; in live rooms
  # (no `page_path`) `LocaleController` falls back to the `Referer`.
  defp language_href(locale, nil), do: ~p"/locale/#{locale}"

  defp language_href(locale, page_path),
    do: ~p"/locale/#{locale}?#{[return_to: localized_path(locale, page_path)]}"
  ...
  Takes the request `conn` so every link can be resolved to the current
  locale's URL (`/`, `/en`, `/fr`, `/es`).
  """
  attr(:class, :string, default: nil)
  attr(:conn, :map, required: true, doc: "the request conn, for locale-aware links")

  def app_header(assigns) do
    locale = assigns.conn.assigns[:locale] || Locales.default()

    assigns =
      assigns
      |> assign(:class, assigns.class || "")
      |> assign(:locale, locale)
      |> assign(:page_path, base_path(assigns.conn.request_path))
    ...
    <header class={[
      ...
      <div class="max-w-6xl mx-auto w-full px-6 py-6 flex justify-between items-center">
        <.link navigate={localized_path(@locale, "/")} class="flex items-center gap-2">
          <svg
        ...
        <nav class="hidden md:flex items-center gap-2 font-semibold">
          <.button navigate={localized_path(@locale, ~p"/how-it-works")}>
            <.icon name="hero-book-open" class="size-5" /> {gettext("Features")}
          </.button>
          <.button navigate={localized_path(@locale, ~p"/pricing")}>
            <.icon name="hero-currency-dollar" class="size-5" /> {gettext("Pricing")}
          </.button>
          <.link
            href="https://buymeacoffee.com/lynxplanningpoker"
            target="_blank"
          ...
          <.settings_menu page_path={@page_path} />
        </nav>

        <div class="md:hidden relative" phx-click-away={JS.hide(to: "#mobile-menu")}>
          <button
            type="button"
            class="btn btn-ghost btn-circle"
          ...
          <div
            id="mobile-menu"
            class="hidden absolute right-0 top-full mt-2 z-50 w-64 rounded-2xl border border-base-300 bg-base-100 shadow-xl p-2"
            role="menu"
          >
            <.link
              navigate={localized_path(@locale, ~p"/how-it-works")}
              class="flex items-center gap-2 rounded-xl px-3 py-2.5 font-semibold hover:bg-base-200"
            >
              <.icon name="hero-book-open" class="size-5" /> {gettext("Features")}
            </.link>
            <.link
              navigate={localized_path(@locale, ~p"/pricing")}
              class="flex items-center gap-2 rounded-xl px-3 py-2.5 font-semibold hover:bg-base-200"
            >
              <.icon name="hero-currency-dollar" class="size-5" /> {gettext("Pricing")}
            </.link>
            <.link
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="flex items-center gap-2 rounded-xl px-3 py-2.5 hover:bg-base-200"
            >
              <img src="/images/bmc.png" alt="Buy me a coffee" class="w-28" />
            </.link>
            ...
            <div class="flex items-center justify-between gap-3 px-3 py-2">
              ...
              <.language_switcher id="mobile-language-options" page_path={@page_path} />
            </div>
          </div>
        </div>
      </div>
  ...
  @doc """
  Renders the shared site footer used on every public page (never in a live
  room). Carries the trust links — privacy, security and contact — so they are
  reachable from anywhere on the marketing site.

  Takes the request `conn` so the links resolve to the current locale's URLs.
  """
  attr(:class, :string, default: nil)
  attr(:conn, :map, required: true, doc: "the request conn, for locale-aware links")
  ...
  def footer(assigns) do
    ...
    ~H"""
    <footer class={["border-t border-base-200/60", @class]}>
      <div class="max-w-6xl mx-auto w-full px-6 py-10 flex flex-col items-center gap-6 sm:flex-row sm:justify-between">
        <.link
          navigate={localized_path(@locale, "/")}
          class="flex items-center gap-2 text-sm font-bold hover:opacity-80 transition"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
          ...
          <span>Lynx planning poker</span>
        </.link>

        <nav class="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm font-semibold">
          <.link
            navigate={localized_path(@locale, ~p"/privacy")}
            class="text-base-content/70 hover:text-primary hover:underline"
          >
            {gettext("Privacy")}
          </.link>
          <.link
            navigate={localized_path(@locale, ~p"/security")}
            class="text-base-content/70 hover:text-primary hover:underline"
          >
            {gettext("Security")}
          </.link>
          <a
            href="mailto:contact@lynxplanningpoker.com"
            class="text-base-content/70 hover:text-primary hover:underline"
          >
            {gettext("Contact")}
          </a>
      ...
      <p class="pb-8 text-center text-xs text-base-content/50">
        © {@year} Lynx Planning Poker
      </p>
    </footer>
    """
  end
