defmodule LynxplanningpokerWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    >
      <div class={[
        "w-80 sm:w-96 max-w-80 sm:max-w-96 text-wrap flex items-start gap-3",
        "rounded-xl border border-base-300 bg-base-100 shadow-lg px-4 py-3",
        "border-l-4",
  ...
  attr :class, :any
  attr :variant, :string, values: ~w(primary red)
  slot :inner_block, required: true

  def button(%{rest: rest} = assigns) do
    variants = %{
      "primary" => "btn-soft btn-primary rounded-xl",
      "red" => "btn-error rounded-xl",
      nil => "btn-soft rounded-xl"
    }

    default_class = ["btn", Map.fetch!(variants, assigns[:variant])]

    assigns =
      assign(
        assigns,
        :class,
        if(assigns[:class], do: [assigns.class | default_class], else: default_class)
      )
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LynxplanningpokerWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LynxplanningpokerWeb.Gettext, "errors", msg, opts)
    end
  end
